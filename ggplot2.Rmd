...
```{r include=FALSE}
knitr::opts_chunk$set(warning=FALSE, message=FALSE, out.width="50%")
```
...
```{r, message=FALSE, eval=TRUE, width="50"} 

stops <- readr::read_csv("./data/opp-search-marijuana_state.csv") %>% 
  dplyr::filter(state %in% c("WA", "CO")) %>% 
  dplyr::mutate(legalization_status = ifelse(quarter <= "2013-01-01", "pre","post"),
